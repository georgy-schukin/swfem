...
CFExecutor::CFExecutor(const unsigned int& nt, const int& rank) : num_of_threads(nt) {
...
/*void CFExecutor::execCF(CompFragment *cf) {
	...
	unsigned int thread_id = scheduler->scheduleCF(cf); // get thread id from scheduler
...
void CFExecutor::execCFs(const CompFragmentPtrArray& cfs) {	
	...
	typedef std::map<unsigned int, CompFragmentPtrArray> CompFragmentGroupMap;
