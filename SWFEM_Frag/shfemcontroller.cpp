...
#define FOREACH(i,j) for (unsigned int i = 0; i < fy; i++) for (unsigned int j = 0; j < fx; j++)

void SHFEMController::exec(const unsigned int& mesh_size_x, const unsigned int& mesh_size_y, const unsigned int& fragment_num_x, const unsigned int& fragment_num_y, const unsigned int& num_of_steps) {
	...
	const unsigned int fx = mesh_data.nx.size();	// num of fragments on this node by X
	const unsigned int fy = mesh_data.ny.size();	// num of fragments on this node by Y
	...
	FOREACH(i, j) {
		const unsigned int& stx = mesh_data.start_x[j];
		const unsigned int& sty = mesh_data.start_y[i];
		const unsigned int& nx = mesh_data.nx[j];
		const unsigned int& ny = mesh_data.ny[i];
	...
	unsigned int red_id = 0;
	unsigned int iter = 0;
	for (unsigned int s = 1; s <= num_of_steps; s++) {	// for each time step		
...
void SHFEMController::innerUpdate(DFArray2D<Data>& dt) {
	for (unsigned int i = 0; i < dt.getSizeByY() - 1; i++) 
	for (unsigned int j = 0; j < dt.getSizeByX(); j++) {
		const unsigned int num = mesh_data.nx[j]; 
		const unsigned int start = (mesh_data.ny[i] - 1)*mesh_data.nx[j];
		addCF(new CFJacobyUpdate(dt(i, j), dt(i + 1, j), start, 1, 0, 1, num)); // Jacoby update (vertical, update hor lines)						
	}

	for (unsigned int i = 0; i < dt.getSizeByY(); i++) 
	for (unsigned int j = 0; j < dt.getSizeByX() - 1; j++) {
		const unsigned int num = mesh_data.ny[i]; 
		const unsigned int start = mesh_data.nx[j] - 1;
		const unsigned int step_l = mesh_data.nx[j];
		const unsigned int step_r = mesh_data.nx[j + 1];
...
void SHFEMController::outerUpdateTop(DFArray2D<Data>& dt, DFArray1D<Data>& send, DFArray1D<Data>& recv) {
	...
	const unsigned int i = 0; // top line of fragments
	for (unsigned int j = 0; j < send.getSize(); j++) {
...
void SHFEMController::outerUpdateBottom(DFArray2D<Data>& dt, DFArray1D<Data>& send, DFArray1D<Data>& recv) {
	...
	const unsigned int i = dt.getSizeByY() - 1; // bottom line
	for (unsigned int j = 0; j < send.getSize(); j++) {
		const unsigned int start = (mesh_data.ny[i] - 1)*mesh_data.nx[j];
		const unsigned int num = mesh_data.nx[j];
...
void SHFEMController::distribute(const unsigned int& mesh_size_x, const unsigned int& mesh_size_y, const unsigned int& fragment_num_x, const unsigned int& fragment_num_y, MeshData& m_data) {
	...
	unsigned int f_start_y, f_end_y;
