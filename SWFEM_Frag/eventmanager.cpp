...
/*void EventManager::registerEventCF(size_t event_id, EventCompFragment *cf) {
...
void EventManager::registerEventCFs(const size_t& event_id, const EventCompFragmentPtrArray& cfs) {
	if (events.find(event_id) == events.end()) {	// new reduction		
		size_t evnt_id = event_id;
...
void EventManager::waitForEvent(const size_t& event_id) {
...
/*void EventManager::onCFDone(CompFragment *cf) {
	if (cf->isEventCF()) {
		const size_t event_id = ((EventCompFragment*)cf)->getEventId();
		events.at(event_id).changeSize(-1);	
	}	
}*/

void EventManager::onCFsDone(const CompFragmentPtrArray& cfs) {
	typedef std::map<size_t, int> EventsDoneMap;
